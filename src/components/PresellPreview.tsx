...
import { Download, Eye, Code, FileCode, ImageIcon, Sparkles } from 'lucide-react';
...
export function PresellPreview({ data, onDownload }: PresellPreviewProps) {
  ...
  const hasNoImage = !data.productImageUrls || data.productImageUrls.length === 0;
    ...
    <Card className="shadow-2xl border-none overflow-hidden bg-white h-full flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50/50 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-lg">Preview da Página</CardTitle>
      ...
      <Tabs defaultValue="preview" className="flex-1 flex flex-col overflow-hidden">
        <div className="px-6 py-2 bg-white border-b flex justify-between items-center shrink-0">
          ...
          {hasNoImage && (
            <div className="flex items-center gap-1.5 text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded border border-amber-100 animate-pulse">
