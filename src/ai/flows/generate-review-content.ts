...
`;
    ...
    // 3. BONUSES - validar contra productInfo e normalizar estrutura
    ...
    if (!hasRealBonuses) {
      delete parsed.bonuses;
    } else if (parsed.bonuses) {
      parsed.bonuses.headline = parsed.bonuses.headline || 'Free Bonuses Included';
      parsed.bonuses.items = (parsed.bonuses.items || [])
        .filter((item: any) => item?.title)
        .map((item: any) => ({ title: item.title || '', description: item.description || '' }));
      if (parsed.bonuses.items.length === 0) delete parsed.bonuses;
    }
